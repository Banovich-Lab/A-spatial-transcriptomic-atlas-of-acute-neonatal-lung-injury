...
######## EPITHELIAL, ENDOTHELIAL, AND MESENCHYMAL LINEAGES ONLY ########

#### SUBSETTING OBJECT AND MAKING GENE FILTERING THRESHOLD ----

## Subset out immune
run2_final_nonimm <- subset(run2_final, subset = Lineage != "Immune")

## Subset
sobj_sub <- run2_final_nonimm[,run2_final_nonimm$Sample_combined %in% 
...
saveRDS(min3Per1k_prop20,"/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/min3Per1k_prop20_2025_05_28.rds")
...
rownames(meta_info) <- gsub("_","-",meta_info$Sample_id)

colData(sobh_sub_aggr_sce) <- cbind(colData(sobh_sub_aggr_sce), 
                                    meta_info[sobh_sub_aggr_sce$sample_id,])

saveRDS(sobh_sub_aggr_sce,"/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/sobh_sub_aggr_sce_2025_05_28.rds")
...
for(test_ct in unique(sobh_sub_aggr_sce$CT_final)){
  ...
  dir.create(file.path("/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/2025-05-28/min3Per1k_prop20/",test_ct),
             recursive = TRUE)
  ...
  write.csv(sig_genes_gage,file.path("/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/2025-05-28/min3Per1k_prop20/",test_ct,
                                     paste0("/test_g_age_genes",
                                            test_ct,"_v_plot.csv")))
  saveRDS(efit, file.path("/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/2025-05-28/min3Per1k_prop20/",test_ct,
                          paste0("/efit.",
                                 test_ct,".rds")))
  ...
  write.csv(sig_genes_life_span,file.path("/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/2025-05-28/min3Per1k_prop20/",test_ct,
                                          paste0("test_life_span_genes",
                                                 test_ct,"_v_plot.csv")))
  
  write.csv(efit$coefficients,
            file.path("/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/2025-05-28/min3Per1k_prop20/",test_ct,
...
id_list <- c("Activated fibroblasts", "Adventitial fibroblasts", "AKR1C1+ & AKR1C2+", "Arterial", "AT1", "AT2", "Basal", 
             "Capillaries", "Fibroblasts", "Immature AT2", "Lymphatic endothelial", "Multiciliated", "MyoFB", "Pericytes",
             "PNEC", "Proliferating airway", "Proliferating basal", "Proliferating endothelial", "Proliferating Immature alveolar",
             "Secretory 3A2+ & 1A1+", "Secretory MUC5B+", "SMC", "Transitional AT2", "Venous")

## Loading in all .csv files with stats from LM analysis
ga_test_stats <- list.files(path = "/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/2025-05-28/min3Per1k_prop20/", 
                            pattern = "^test_g_age_genes",
                            recursive = TRUE, 
                            full.names = TRUE) 

ls_test_stats <- list.files(path = "/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/2025-05-28/min3Per1k_prop20/", 
                            pattern = "^test_life_span_genes",
                            recursive = TRUE, 
                            full.names = TRUE) 

non_int_test_stats <- list.files(path = "/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/2025-05-28/min3Per1k_prop20/", 
...
saveRDS(sobh_sub_aggr_sce,"/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/sobh_sub_aggr_sce_path_score_2025_05_28.rds")
...
for(test_ct in unique(sobh_sub_aggr_sce$CT_final)) {
  ...
  write.csv(sig_genes_dx, file.path("/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/2025-05-28/min3Per1k_prop20/",test_ct,
...
## Loading in all .csv files with stats from LM analysis
dx_test_stats <- list.files(path = "/scratch/smallapragada/run2/lm_regression_ga_ls_ruqian_2025_04_11/2025-05-28/min3Per1k_prop20/", 
...
write_csv(all_stats_morphed, "/scratch/smallapragada/gene_ct_nonimmune.csv")
